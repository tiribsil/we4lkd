...
def feedback_new_topics(normalized_target_disease: str, max_new_topics: int = 4, max_topics: int = 4):
    ...
    'topics_of_interest.txt' file. It ensures that no duplicate topics are added
    and limits the number of new topics added in a single run.

    Args:
        normalized_target_disease (str): The normalized name of the target
                                         disease, used for constructing file paths.
        max_new_topics (int): The maximum number of new topics to add in this run.
                                Defaults to 2.
    """
    ...
    try:
        ...
        if len(existing_topics) >= 4:
            print(f"Topics file already has more than {max_topics} lines, skipping feedback.")
            return
        ...
        for topic in potential_new_treatments:
            if topic in existing_topics: continue
            existing_topics.add(topic)
            new_topics_added_count += 1
            if new_topics_added_count >= max_new_topics: break

        # Write the updated and sorted list back to the topics file.
        with open(topics_file, 'w', encoding='utf-8') as f:
            for topic in sorted(list(existing_topics)):
                f.write(f"{topic}\n")

        print(f"Feedback complete: {new_topics_added_count} new potential treatments added.")
        print(f"Total topics of interest are now {len(existing_topics)}.")
...
def main_pipeline():
    ...
    for current_year in range(start_year, end_year + 1):
        ...
        generate_latent_knowledge_report(target_disease, normalized_target_disease, delta_threshold=0.01)
